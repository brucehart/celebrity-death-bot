...
export type XOAuthOwner = { email: string; sessionId: string };
...
async function encryptJson(env: Env, obj: unknown): Promise<{ iv: string; ct: string } | null> {
...
async function decryptJson(env: Env, payload: { iv: string; ct: string }): Promise<unknown> {
	const keyB64 = env.X_ENC_KEY;
	if (!keyB64) return null;
	const key = await importAesKeyFromBase64(keyB64);
...
async function saveSession(env: Env, state: string, codeVerifier: string, owner: XOAuthOwner) {
	const expiresAt = nowSec() + OAUTH_SESSION_SECONDS;
	await env.DB.batch([
		env.DB.prepare(`DELETE FROM oauth_sessions WHERE expires_at <= ?1`).bind(nowSec()),
		env.DB.prepare(
			`INSERT INTO oauth_sessions(state, code_verifier, owner_email, session_id, expires_at, created_at)
       VALUES(?1, ?2, ?3, ?4, ?5, CURRENT_TIMESTAMP)`,
		).bind(state, codeVerifier, owner.email.toLowerCase(), owner.sessionId, expiresAt),
	]);
}

async function loadAndDeleteSession(env: Env, state: string, owner: XOAuthOwner): Promise<string | null> {
	const row = await env.DB.prepare(
		`DELETE FROM oauth_sessions
      WHERE state = ?1 AND owner_email = ?2 AND session_id = ?3 AND expires_at > ?4
      RETURNING code_verifier`,
	)
		.bind(state, owner.email.toLowerCase(), owner.sessionId, nowSec())
...
function parseTokenPayload(value: unknown): TokenPayload | null {
	if (!isRecord(value) || typeof value.access_token !== 'string' || !value.access_token) return null;
	return {
		...
		refresh_token_expires_in: typeof value.refresh_token_expires_in === 'number' ? value.refresh_token_expires_in : undefined,
	};
}
...
async function getTokens(env: Env): Promise<{ payload: TokenPayload; expires_at: number | null } | null> {
	const row = await env.DB.prepare(`SELECT data, expires_at FROM oauth_tokens WHERE provider = 'x'`).first<{
		data: string;
	...
	try {
		...
		if (!isRecord(parsed) || typeof parsed.iv !== 'string' || typeof parsed.ct !== 'string') return null;
		const payload = parseTokenPayload(await decryptJson(env, { iv: parsed.iv, ct: parsed.ct }));
		return payload ? { payload, expires_at: row.expires_at ?? null } : null;
	} catch {
		return null;
	}
}
...
async function refreshIfNeeded(env: Env): Promise<string | null> {
	...
	if (!res.ok) {
		console.warn('X refresh failed', res.status);
		return null;
	}
	try {
		const payload = parseTokenPayload(JSON.parse(await readResponseTextBounded(res, MAX_WEBHOOK_BODY_BYTES)));
		if (!payload) return null;
		if (!payload.refresh_token) payload.refresh_token = refresh;
...
export async function xOauthStart(env: Env, baseUrl: string, owner: XOAuthOwner): Promise<Response> {
	if (!env.X_CLIENT_ID || !env.X_ENC_KEY) return new Response('X OAuth is not configured', { status: 503 });
	const redirectUri = new URL('/x/oauth/callback', baseUrl).toString();
	const state = randB64Url(24);
	const verifier = randB64Url(64);
	const challenge = await sha256B64Url(verifier);
	await saveSession(env, state, verifier, owner);
...
export async function xOauthCallback(env: Env, requestUrl: string, baseUrl: string, owner: XOAuthOwner): Promise<Response> {
	const url = new URL(requestUrl);
	const code = url.searchParams.get('code');
	const state = url.searchParams.get('state') || '';
	if (!code || !state || code.length > 2_048 || state.length > 256) return new Response('Missing or invalid code/state', { status: 400 });
	const verifier = await loadAndDeleteSession(env, state, owner);
	...
	const res = await fetchWithRetry(TOKEN_URL, { method: 'POST', headers, body: params.toString() }, { retries: 1, timeoutMs: 15000 });
	if (!res.ok) {
		console.warn('X token exchange failed', res.status);
		return new Response('X token exchange failed', { status: 502 });
	}
	let payload: TokenPayload | null;
	try {
		payload = parseTokenPayload(JSON.parse(await readResponseTextBounded(res, MAX_WEBHOOK_BODY_BYTES)));
	} catch {
		return new Response('X token exchange failed', { status: 502 });
	}
	if (!payload) return new Response('X token exchange failed', { status: 502 });
	await saveTokens(env, payload);
...
export function buildXStatus(
	{ name, age, description, cause, wiki_path, link_type }: PostInput,
	opts?: { includeWikipediaLink?: boolean },
): string {
...
export async function postToXIfConfigured(env: Env, text: string): Promise<void> {
	...
	try {
		...
		if (!res.ok) {
			console.warn('X post failed', res.status);
		}
	} catch (err) {
